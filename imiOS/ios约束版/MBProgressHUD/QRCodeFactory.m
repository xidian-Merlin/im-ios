...
@implementation QRCodeFactory

+ (UIImage *)qrImageForIntNumber:(int)intNumber imageSize:(CGFloat)Imagesize logoImageSize:(CGFloat)waterImagesize
{
    CIFilter *filter = [CIFilter filterWithName:@"CIQRCodeGenerator"];
    [filter setDefaults];
    NSString *string = [NSString stringWithFormat:@"%d",intNumber];
//    NSString *string = @"把logo图画到生成的二维码图片上，注意尺寸不要太大（最大不超过二维码图片的30%），太大会造成扫不出来";
    NSLog(@"string:%@",string);
    NSData *data = [string dataUsingEncoding:NSUTF8StringEncoding];
    [filter setValue:data forKey:@"inputMessage"];//通过kvo方式给一个字符串，生成二维码
    [filter setValue:@"H" forKey:@"inputCorrectionLevel"];//设置二维码的纠错水平，越高纠错水平越高，可以污损的范围越大,H level容许30%的污损
...
- (UIImage *)createNonInterpolatedUIImageFormCIImage:(CIImage *)image withSize:(CGFloat)size waterImageSize:(CGFloat)waterImagesize
{
    ...
    if (nil == user.headerImage) {
        waterimage = [UIImage imageNamed:@"tn.9.png"];
    }
    else {
        waterimage = user.headerImage;
   ...
   // UIImage *waterimage = [UIImage imageNamed:@"网信院徽128×128"];
