...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        faceButton.backgroundColor = [UIColor blackColor];
        ...
        self.plistFileName = @"emoticons";        //默认显示表情图片
        [self tapButton1:nil];    //fangfa
...
{
    [self.headerScrollView removeFromSuperview]; //移除当前view
    ...
    CGFloat width = (headers.count/(scrollHeight/30)+1.3)*30;
    ...
    for (int i = 0; i < headers.count; i ++) {
        ...
        [face setFaceBlock:^(UIImage *image, NSString *imageText)
         {
             copy_self.block(image, imageText);//将点击的表情继续上传，将在文本框中显示文字，并存入数据库
         }];
