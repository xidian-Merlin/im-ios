...
-(void) addSubview
{
    ...
    [self.functionView setFunctionBlock:^(UIImage *image, NSString *imageText)
     {
         ...
         // NSString *str =  copy_self.sendTextView.text;
         BOOL isequal = [str isEqualToString:@""];
         if (isequal == NO) {
             copy_self.sendTextButton.hidden = NO;
             copy_self.moreButton.hidden = YES;
         }
         else  {
             copy_self.sendTextButton.hidden = YES;
             copy_self.moreButton.hidden = NO;
         }
    ...
    [self.moreView setMoreBlock:^(NSInteger index) {
        ...
        copy_self.extendBlock((int)index);//继续向上层传值
    }];
...
-(void)addConstraint
{
    ...
    self.sendTextButton.translatesAutoresizingMaskIntoConstraints = NO;
    
    NSArray *sendTextButtonH = [NSLayoutConstraint constraintsWithVisualFormat:@"H:[_sendTextButton(45)]-4-|" options:0 metrics:0 views:NSDictionaryOfVariableBindings(_sendTextButton)];
    [self addConstraints:sendTextButtonH];
    
    NSArray *sendTextButtonV = [NSLayoutConstraint constraintsWithVisualFormat:@"V:|-9-[_sendTextButton(30)]" options:0 metrics:0 views:NSDictionaryOfVariableBindings(_sendTextButton)];
    [self addConstraints:sendTextButtonV];
    ...
    //  [[NSNotificationCenter defaultCenter]addObserver:self selector:@selector(textChangedExi:) name:UITextViewTextDidChangeNotification object:nil];
    
    //  [UITextView addObserver:self forKeyPath:@"text" options:NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld context:nil];
}
...
//点击sendTextButton 按钮，发送消息
-(void)tapsendTextButton:(id)sender
{
    ...
    NSString *str =  self.sendTextView.text;
    BOOL isequal = [str isEqualToString:@""];
    if (isequal == NO) {
        ...
        if (longPress.state == UIGestureRecognizerStateEnded) {
            if (i == 1)
            {
                ...
                if (cTime > 1)
                {
                    //如果录制时间<1 不发送
                    NSLog(@"发出去");
                    NSLog(@"打印%@",_audioPlayURL);
                    
                    NSData *soundData = [NSData dataWithContentsOfFile:_string];
                    NSLog(@"语音数据:%lu",(unsigned long)soundData.length);
                    [self.audioRecorder stop];
                    self.urlBlock(_fileName);  //回传文件名
...
-(void)audioInit
{
    ...
    NSString *strUrl = [_getFolder getFolder];
    
    _fileName = [NSString stringWithFormat:@"Voice/%ld.aac", (long)[[NSDate date] timeIntervalSince1970]];//以记录时间为文件名
    
    _string = [NSString stringWithFormat:@"%@/%@", strUrl, _fileName];
    
    NSURL *url = [NSURL fileURLWithPath:_string];
    _audioPlayURL = url;   //录音文件的目录
...
- (void)detectionVoice
{
    ...
    //把声音的音量传给调用者
    self.volumeBlock(lowPassResults);//回调机制，可以保存参数
}


//通过屏幕旋转改变function的高度
-(void) changeFunctionHeight: (float) height     //frame 的属性并不能直接改变
{
...
//轻击sendText切换键盘，但是在模拟器上不容易体现出戏功能
...
-(void)textViewDidChange:(UITextView *)textView   //这是代理协议方法
{
    CGSize contentSize = self.sendTextView.contentSize;
    NSString *str =  self.sendTextView.text;
    BOOL isequal = [str isEqualToString:@""];
...
-(void)tapMoreButton:(UIButton *) sender
{
    if ([self.sendTextView.inputView isEqual:self.moreView])
    {
        ...
        [self.sendTextView reloadInputViews];//不设置的话将会出现系统键盘
    }
    else
    {
    ...
    if (self.sendTextView.hidden == YES) {       //在录音状态下的点击
        self.sendTextView.hidden = NO;
        self.sendVoiceButton.hidden = YES;
