...
@interface gChatDb()   //为每一个用户建一堆消息表，每个表对应一个会话消息
...
-(instancetype) init
{
    ...
    if (self) {
        ...
        _tableName = [NSString stringWithFormat:@"%@%@", @"xdgc", nowUser.userName];
...
- (BOOL)createTableWithGroupId:(int)groupId
{
    ...
    NSString *tableName0 = [NSString stringWithFormat:@"%@%d", _tableName, groupId];
...
/*- (void)loadMore         //WHERE pk > %d limit 10    WHERE pk < %d  order by pk desc limit 10
 {
 static int pk = 100;
 NSArray *array = [User findByCriteria:[NSString stringWithFormat:@"WHERE pk > %d  and pk<%d limit 10",pk-10,pk]];
 pk = ((User *)[array firstObject]).pk;
 [self.data addObjectsFromArray:array];       //将旧的附加到新的末尾
...
-(void) updatewihtId:(int) messageId  newFinishFlag:(int)flag   
{
    ...
    [_shareInstance.dbQueue inDatabase:^(FMDatabase *db) {
        ...
        NSString *sql = [NSString stringWithFormat:@"UPDATE %@ SET %@ WHERE id = ?;", tableName1, keyString];
