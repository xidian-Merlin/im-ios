...
- (Analysis)analysisReturnData
{
    Analysis analysis = (id)^(NSData* data)
    {
        ...
        if (resultCode == 0) {
            ...
            user = [[IMUserEntity alloc]initWithName:name avatar:avatar nick:nick];
        }else{
            user = nil;
        }
        return user;
...
- (Package)packageRequestObject
{
    Package package = (id)^(id object,uint32_t packageID)
    {
        ...
        NSLog(@"要查找的用户ID：%d",userID);
        DataOutputStream *dataout = [[DataOutputStream alloc] init];
        [dataout writeTcpProtocolHeaderWithCommandID:IM_USER subcommandID:IM_USER_QUERY packageID:packageID];
