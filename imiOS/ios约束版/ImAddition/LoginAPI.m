...
- (Analysis)analysisReturnData
{
    Analysis analysis = (id)^(NSData* data)
    {
        ...
        NSString *result = [ServerFeedbackConverter convertServerFeedbackWithResultCode:resultCode];
        NSArray *response = @[result];
        
        if (resultCode == 0) {
            int userID = [datain readInt];
            NSLog(@"用户ID:%04x",userID);
...
- (Package)packageRequestObject
{
    Package package = (id)^(id object,uint32_t packageID)
    {
        ...
        NSLog(@"用户名密码长度：%d",plainCharLength);
        ...
        [dataout writeTcpProtocolHeaderWithCommandID:IM_ACCOUNT subcommandID:IM_ACC_LOG_IN packageID:packageID];
