...
@implementation ChangeRemarkAPI
/**
 *  请求超时时间
 *
 *  @return 超时时间
...
- (Analysis)analysisReturnData
{
    Analysis analysis = (id)^(NSData* data)
    {
        ...
        if (resultCode == 0) {
            resultString = @"ChangeSuccess";
        }else{
            resultString = @"ChangeFailure";
        }
...
- (Package)packageRequestObject
{
    Package package = (id)^(id object,uint32_t packageID)
    {
        ...
        NSString *remark = object[1];
        NSLog(@"要修改的用户ID：%d",userID);
        DataOutputStream *dataout = [[DataOutputStream alloc] init];
        [dataout writeTcpProtocolHeaderWithCommandID:IM_USER subcommandID:IM_USER_MODITY packageID:packageID];
        [dataout writeInt:userID];
        [dataout writeUTF:remark];
