...
- (Analysis)analysisReturnData
{
    Analysis analysis = (id)^(NSData* data)
    {
        ...
        IMGroupEntity *group = [[IMGroupEntity alloc] init];
        if (resultCode == 0) {
            NSLog(@"获取群信息成功!");
            int groupID = [datain readInt];
            NSString* groupName = [datain readUTFWithInt];
            int memberNumber = [datain readInt];
            group = [[IMGroupEntity alloc]initWithGroupID:groupID name:groupName memberNumber:memberNumber];
...
- (Package)packageRequestObject
{
    Package package = (id)^(id object,uint32_t packageID)
    {
        ...
        [dataout writeTcpProtocolHeaderWithCommandID:IM_GROUP subcommandID:IM_GRP_INFO packageID:packageID];
